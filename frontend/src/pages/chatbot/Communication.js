    ...
    const fetchLlmSettings = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          console.log("📥 [Communication] Fetched LLM Settings:", data);
          if (data.llm_model) setSelectedModel(data.llm_model);
        }
      } catch (e) {
        console.error("Error fetching LLM settings in Communication:", e);
      }
    };
    fetchLlmSettings();
  }, [userEmail]);
  ...
  const sendMessage = async (altText = null, payload_index = null) => {  //Tanmey Start
    ...
    socket.onmessage = (event) => {
      ...
      if (data.type === "token") {
        ...
        if (!assistantId) {
          // First token: create the bubble now (not before)
          assistantId = generateCustomUUID();
          setIsTyping(false);
          setMessages(prev => [
            ...prev,
            { id: assistantId, role: 'assistant', content: accumulatedReply }
          ]);
        } else {
          setMessages(prev => {
            const updated = [...prev];
            for (let i = updated.length - 1; i >= 0; i--) {
              if (updated[i].role === 'assistant') {
                updated[i] = { ...updated[i], content: accumulatedReply };
                break;
              }
            }
            return updated;
      ...
      if (data.type === "meta") {
        console.log("📥 Meta frame received:", data);
        if (data.message_ids?.assistant) {
          const realId = data.message_ids.assistant;
          setMessages(prev => {
            const updated = [...prev];
            for (let i = updated.length - 1; i >= 0; i--) {
              if (updated[i].role === 'assistant') {
