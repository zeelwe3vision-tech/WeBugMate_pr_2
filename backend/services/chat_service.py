...
#JONCY START
def select_relevant_history(conv_hist, max_messages=5):
    """
    Select important conversation messages instead of just last N messages.
    """
    if not conv_hist:
        return []

    keywords = [
        "project", "database", "api", "backend", "frontend",
        "bug", "error", "issue", "task", "feature"
    ]
...
async def handle_common_chat(data, current_user, stream: bool = False):
    try:
        ...
        if intent in ["company_info", "documentation", "knowledge"]:
            doc_context = get_context(user_query)
        #JONCY END
    ...
    f"Known facts: {facts if facts else 'None'}.\n"
        ...
        #JONCY START
        # Only inject DB schema when query likely involves database tables
        if intent in ["table_query", "database_query", "project_details", "all_projects"]:
            tables_json = json.dumps(
                {table: list(cols) for table, cols in TABLES.items()},
                indent=2
            )
        ...
        if wants_table:
            ...
            Then suggest two short follow-up questions.
        ...
        #JONCY START
        system_prompt = f"""
            You are an AI assistant for We3Vision.
        ...
        #JONCY START
        relevant_history = select_relevant_history(conv_hist)
        ...
        if stream and not wants_table:
            async def token_generator():
                ...
                if not collected_reply:
                    collected_reply = "⚠ AI service temporarily unavailable."

                final_safe_reply = collected_reply

                # safely process response
                try:
                    final_safe_reply, _ = _process_common_response(
                        collected_reply,
                        user_query,
                        normalized_query,
                        user_email,
                ...
                yield {
                    ...
                    "clarifications": You_Might_Also_Ask,
                    "multi_clarification": True if You_Might_Also_Ask else False
                }
        ...
        if len(match) > 1:
            ...
            for line in suggestion_lines:
                ...
                if line.startswith("-"):
                    ...
                    You_Might_Also_Ask.append(line.split(".", 1)[1].strip())
...
def _process_common_response(
    ...
    if is_tech_related_query(user_query):
        if not validate_code_response(ethical_reply):
            ...
            #JONCY START
    final_safe_reply = ethical_reply
    #JONCY END
    #JONCY COMMENTED 
